...
@booksrouter.get("/books", response_model=List[Book])
async def get_all_books():
    return books

@booksrouter.post("/books", status_code=status.HTTP_201_CREATED)
async def create_book(book_data: Book) -> dict:
    new_book = book_data.model_dump()
    books.append(new_book)
    return new_book

@booksrouter.get("/books/{book_id}")
async def get_book(book_id: int) -> dict:
    for book in books:
            return book
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Book not found")

@booksrouter.put("/books/{book_id}")
...
@booksrouter.delete("/books/{book_id}")
